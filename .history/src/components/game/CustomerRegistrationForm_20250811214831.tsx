...
export default function CustomerRegistrationForm({ gameId }: { gameId: string }) {
    ...
    // dynamicSchema is always a Zod schema with all possible fields, but we can refine it for validation
    const [dynamicSchema, setDynamicSchema] = useState(baseFormSchema);
        ...
        useEffect(() => {
                ...
                const unsubscribe = onSnapshot(gameRef, (docSnap) => {
                        if (docSnap.exists()) {
                                ...
                                // Refine the schema for validation, but always keep all possible fields
                                let schema = baseFormSchema;
                                if (isPhoneRequired) {
                                    schema = schema.extend({
                                        phone: z.string().min(6, 'Por favor, introduce un número de teléfono válido.')
                                    });
                                }
                                if (instagramProfile) {
                                    schema = schema.extend({
